...
aws ssm put-parameter --name $default_ssm_path/$stack_id/$key --region $region --type String --value Starting --overwrite
aws ssm put-parameter --name $default_ssm_path/$stack_id/$key --region $region --type String --value StartingYumUpdates --overwrite
yum install -y gcc glibc glibc-common wget unzip httpd php gd gd-devel perl postfix
cd /tmp
aws ssm put-parameter --name $default_ssm_path/$stack_id/$key --region $region --type String --value DownloadingNagios --overwrite
wget -O nagioscore.tar.gz https://github.com/NagiosEnterprises/nagioscore/archive/nagios-4.4.5.tar.gz
tar xzf nagioscore.tar.gz
cd /tmp/nagioscore-nagios-4.4.5/
aws ssm put-parameter --name $default_ssm_path/$stack_id/$key --region $region --type String --value DeployingNagios --overwrite
./configure
make all
...
cd /tmp
aws ssm put-parameter --name $default_ssm_path/$stack_id/$key --region $region --type String --value DownloadingPlugins --overwrite
...
aws ssm put-parameter --name $default_ssm_path/$stack_id/$key --region $region --type String --value DeployingPlugins --overwrite
./tools/setup
./configure
make
make install
aws ssm put-parameter --name $default_ssm_path/$stack_id/$key --region $region --type String --value StartingServices --overwrite
systemctl restart httpd.service
systemctl restart nagios.service
aws ssm put-parameter --name $default_ssm_path/$stack_id/$key --region $region --type String --value Done --overwrite
"""
...
# Helper functions
# =====================================
def build_instance(model:ResourceModel, session, stack_id:str):
    ...
    try:
        ...
        # start EC2 instance
        LOG.info(f"...Starting EC2 instance {instance_name} with {image_id} on {instance_type} instance, subnet {subnet_id} and security groups {sg_ids}")
        user_data = ec2_user_data.replace('$stack_id', stack_id.replace(':','-').replace('/','-')).replace('$default_ssm_path', default_ssm_path).replace('$key', const_key_status)
        ...
        model.PolicyArn = policy_arn
        model.InstanceProfile = instance_profile_name
...
def check_instance_state(model:ResourceModel, session, stack_id):
    ...
    try:
        ...
        if state_code == 16:
            ...
            status = ssm_parameter_action(ssm_action_get, session, stack_id, const_key_status)
            if status == 'Done':
                # we're done
                ...
                LOG.info(f"...{msg}")
                progress = ProgressEvent(status=OperationStatus.IN_PROGRESS, callbackDelaySeconds=default_callback_period, resourceModel=model, message=msg)
        elif state_code == 0:
...
def ssm_parameter_action(action, session, stack_id, key, value=None):

    ssm_client = session.client('ssm')
    ssm_name = f"{default_ssm_path}/{stack_id.replace(':','-').replace('/','-')}/{key}"
    
    if action == ssm_action_put:
        ssm_value = value
        ssm_client.put_parameter(Name=ssm_name, Value=ssm_value, Type='String', Overwrite=True)
...
def create_handler(
    ...
    if model.Id is None:
        ...
        progress = build_instance(model, session, request.stackId)

        LOG.info(f"...Storing role, policy and instance profile to SSM")
        ssm_parameter_action(ssm_action_put, session, request.stackId, const_key_policy_arn, model.PolicyArn)
        ssm_parameter_action(ssm_action_put, session, request.stackId, const_key_role, model.Role)
        ssm_parameter_action(ssm_action_put, session, request.stackId, const_key_instance_profile, model.InstanceProfile)

    else:
        LOG.info(f"...model.Id is {model.Id}, checking instance state")
        progress = check_instance_state(model, session, request.stackId)
        if progress.status == OperationStatus.SUCCESS:
            # Store server id information in SSM - needed for read after delete
            LOG.info(f"...instance is running, storing instance id in ssm")
            ssm_parameter_action(ssm_action_put, session, request.stackId, const_key_instance_id, model.Id)

            LOG.info(f"...storing IP and URL in SSM")
            ssm_parameter_action(ssm_action_put, session, request.stackId, const_key_IP, model.IP)
            ssm_parameter_action(ssm_action_put, session, request.stackId, const_key_URL, model.URL)
...
def delete_handler(
    ...
    try:
        ...
        instance_profile = ssm_parameter_action(ssm_action_get, session, request.stackId, const_key_instance_profile)
        role = ssm_parameter_action(ssm_action_get, session, request.stackId, const_key_role)
        policy_arn = ssm_parameter_action(ssm_action_get, session, request.stackId, const_key_policy_arn)
        ...
        ssm_parameter_action(ssm_action_delete, session, request.stackId, const_key_instance_id)
        ssm_parameter_action(ssm_action_delete, session, request.stackId, const_key_instance_profile)
        ssm_parameter_action(ssm_action_delete, session, request.stackId, const_key_role)
        ssm_parameter_action(ssm_action_delete, session, request.stackId, const_key_policy_arn)
        ssm_parameter_action(ssm_action_delete, session, request.stackId, const_key_IP)
        ssm_parameter_action(ssm_action_delete, session, request.stackId, const_key_URL)
        ssm_parameter_action(ssm_action_delete, session, request.stackId, const_key_status)
...
def read_handler(
    ...
    model = request.desiredResourceState
    try:
        ssm_parameter_action(ssm_action_get, session, request.stackId, const_key_instance_id, model.Id)
        model = request.desiredResourceState
        model.IP = ssm_parameter_action(ssm_action_get, session, request.stackId, const_key_IP)
        model.URL = ssm_parameter_action(ssm_action_get, session, request.stackId, const_key_URL)
        progress = ProgressEvent(status=OperationStatus.SUCCESS, resourceModel=model)
    except:
